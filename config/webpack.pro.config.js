...
var getEntries = function (path) {
  ...
  files.length > 0 && files.forEach(function (filepath) {
    var name = filepath.replace('src/static/pages', '')
...
var entryList = getEntries('src/static/pages/**/!(js)/*.js')
...
module.exports = {
  ...
  module: {
    ...
    rules: [
      ...
      //   },
      //   include: [
      //     /(node_modules\/@lulu\/lubc|src)/,
      //     /(node_modules\/@lu\/utilities|src)/
      //   ]
      // },
      // {
      //   test: /\.(png|jpg|jpeg|gif)$/,
      //   use: `${require.resolve(
      //     'url-loader'
      //   )}?limit=100&&name=images/[name].[hash:6].[ext]`
      // },
      {
  ...
  plugins: [
    ...
    new CopyWebpackPlugin([
      {
        from: path.resolve(__dirname, '../public/ThirdParty'),
        to: path.resolve(__dirname, '../build/ThirdParty'),
      }
    ]),
